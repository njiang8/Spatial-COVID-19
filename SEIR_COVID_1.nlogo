...
to go
  ...
  ;;S 1, move freely
  if (Stay = False) and (Isolate = False)  or (ticks < 30)[
    ask turtles [rt random 360 fd 1]
    regular
  ]

  ;;S 2, everybody stay at home, works ok
  if (Stay = True) and (Isolate = False) and (ticks > 30)[
    stay-home
    get-infected
    recover
  ]
  ...
  if (Stay = false) and (Isolate = True) and (ticks > 30)[
  ...
  if (Stay = True) and (Isolate = True) and (ticks > 30)[
    ;set isolation treatment area
    ask patches with [pxcor = 49 and pycor = 0]  [set pcolor pink]
...
to spread-1
  ...
  show r0
    ...
    ask n-of r0 turtles with [flag = 1][
...
to get-infected
  ...
  ask turtles with [status = 1][
    ifelse days <= incubation ;icubation days
